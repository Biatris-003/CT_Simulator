...
class SimulatorCTLabApp(QMainWindow):
    ...
    def _build_main_workspace(self):
        ...
        # ---------- NMSE LSR vs Dense ----------
        lsr_nmse_group = QGroupBox("NMSE: LSR vs Dense")
        ...
        # ---------- FBP Reconstruction ----------
        fbp_group = QGroupBox("FBP Reconstruction")
        ...
        ls_group = QGroupBox("Least Squares Reconstruction")
        ls_layout = QVBoxLayout(ls_group)

        self.fig_fbp_sparse, self.ax_fbp_sparse = plt.subplots(facecolor="#1E1E2E")
        self.canvas_fbp_sparse = FigureCanvas(self.fig_fbp_sparse)
        self.canvas_fbp_sparse.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        ls_layout.addWidget(self.canvas_fbp_sparse)
    ...
    def _render_sinograms(self, mu_map, total_i0):
        # Sinograms are rendered in the FBP dialog. This function keeps the
        # cache up-to-date for the main UI's sparse-FBP display.
    ...
    def chosen_spectrum(self, q, energies, kvp, ma, cu, al):
        ...
        # print(f"{timestamp} Spectrum generated: {kvp} kVp, {ma} mA")
